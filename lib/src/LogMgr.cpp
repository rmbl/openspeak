...
namespace openSpeak
{
	...
	LogMgr::LogMgr (const std::string &filename, const Log::Level &minLvl) 
            : mPath ("")
	{
		createLog (filename, minLvl, true);
	}

	LogMgr::LogMgr (const std::string &path, const std::string &filename, 
            const Log::Level &minLvl) : mPath (path)
	{
		createLog (filename, minLvl, true);
	}
	...
	Log* LogMgr::createLog (const std::string &filename, 
            const openSpeak::Log::Level &minLvl, bool _default)
	{
